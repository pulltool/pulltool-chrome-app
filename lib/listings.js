...
function removeListing(id) {
  return getListings().then(function spliceOutListing (listings) {
    var index = listings.findIndex(
      function(listing) { return listing.id == id });
    if (index < 0) return Promise.reject(new Error('Listing not found'));
    else {
      listings.splice(index,1);
      return setListings(listings);
    }
  });
}
