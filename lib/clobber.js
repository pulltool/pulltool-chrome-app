...
function extractToDirectory(exMap, dirEntry) {
  var pendingFileOps = [];
    ...
    return new Promise(function(resolve, reject) {
      ...
      function processEntry(entry) {
        ...
        if (entry.isDirectory) {
          ...
          if (file) {
            // TODO: patching the file content from the archive
            // before writing it
            return pendingFileOps.push(writeFile(entry, file.getBlob())
              .then(function() {
                // Once we've written the file, we can remove it
          ...
          if (pathComponents.length > 1) {
            ...
            return createFile(deepDirEntry, pathComponents[0])
              .then(function(fileEntry){
                return writeFile(fileEntry, file.getBlob());
              });
          }
        }
        diveForFile(dirEntry);
      });
...
clobber.extractToDir = extractToDirectory;
})();
